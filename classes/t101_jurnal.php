<?php namespace PHPMaker2020\p_akuntansi_v1_0; ?>
<?php

/**
 * Table class for t101_jurnal
 */
class t101_jurnal extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 't101_jurnal';
		$this->TableName = 't101_jurnal';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t101_jurnal`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('t101_jurnal', 't101_jurnal', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// period_id
		$this->period_id = new DbField('t101_jurnal', 't101_jurnal', 'x_period_id', 'period_id', '`period_id`', '`period_id`', 3, 11, -1, FALSE, '`period_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->period_id->Required = TRUE; // Required field
		...
		// tipejurnal_id
		$this->tipejurnal_id = new DbField('t101_jurnal', 't101_jurnal', 'x_tipejurnal_id', 'tipejurnal_id', '`tipejurnal_id`', '`tipejurnal_id`', 3, 11, -1, FALSE, '`tipejurnal_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->tipejurnal_id->Required = TRUE; // Required field
		...
		$this->tipejurnal_id->Lookup = new Lookup('tipejurnal_id', 't007_tipejurnal', FALSE, 'id', ["nama","","",""], [], [], [], [], [], [], '', '');
		$this->tipejurnal_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['tipejurnal_id'] = &$this->tipejurnal_id;

		// nomer
		$this->nomer = new DbField('t101_jurnal', 't101_jurnal', 'x_nomer', 'nomer', '`nomer`', '`nomer`', 200, 50, -1, FALSE, '`nomer`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nomer->Required = TRUE; // Required field
		$this->nomer->Sortable = TRUE; // Allow sort
		$this->fields['nomer'] = &$this->nomer;

		// keterangan
		$this->keterangan = new DbField('t101_jurnal', 't101_jurnal', 'x_keterangan', 'keterangan', '`keterangan`', '`keterangan`', 200, 50, -1, FALSE, '`keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->keterangan->Required = TRUE; // Required field
		$this->keterangan->Sortable = TRUE; // Allow sort
		$this->fields['keterangan'] = &$this->keterangan;

		// person_id
		$this->person_id = new DbField('t101_jurnal', 't101_jurnal', 'x_person_id', 'person_id', '`person_id`', '`person_id`', 3, 11, -1, FALSE, '`person_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// createon
		$this->createon = new DbField('t101_jurnal', 't101_jurnal', 'x_createon', 'createon', '`createon`', CastDateFieldForLike("`createon`", 0, "DB"), 135, 19, 0, FALSE, '`createon`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "t102_jurnald") {
			$detailUrl = $GLOBALS["t102_jurnald"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "t101_jurnallist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`t101_jurnal`";
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 't102_jurnald'
		$cascadeUpdate = FALSE;
		$rscascade = [];
		...
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["t102_jurnald"]))
				$GLOBALS["t102_jurnald"] = new t102_jurnald();
			$rswrk = $GLOBALS["t102_jurnald"]->loadRs("`jurnal_id` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB')); 
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["t102_jurnald"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["t102_jurnald"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["t102_jurnald"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 't102_jurnald'
		if (!isset($GLOBALS["t102_jurnald"]))
			$GLOBALS["t102_jurnald"] = new t102_jurnald();
		$rscascade = $GLOBALS["t102_jurnald"]->loadRs("`jurnal_id` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB")); 
		$dtlrows = ($rscascade) ? $rscascade->getRows() : [];

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["t102_jurnald"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["t102_jurnald"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["t102_jurnald"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "t101_jurnaledit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t101_jurnaladd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("t101_jurnalview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t101_jurnalview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "t101_jurnaladd.php?" . $this->getUrlParm($parm);
		else
			$url = "t101_jurnaladd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("t101_jurnaledit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t101_jurnaledit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("t101_jurnaladd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t101_jurnaladd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t101_jurnaldelete.php", $this->getUrlParm());
	}
